...
def dataset_loader(net_name, state = 'Pretrain', purpose = 'Train'):
    if net_name.split('_')[0] == 'Recurrence' and state == 'Pretrain':
        ...
        from temp.Dataset_pickled_recurrence import Recurrence_SAD_DATASET
        train = Recurrence_SAD_DATASET()

    elif net_name.split('_')[0] == 'Recurrence' and state == 'SAD_EGO':
        from temp.Dataset_pickled_recurrence import Recurrence_SAD_EGO_DATASET
        train = Recurrence_SAD_EGO_DATASET()
    
    if purpose == 'Train':
        train, val = data.random_split(train, split_ratio)
        train = data.DataLoader(train, batch_size=2, shuffle = data_shuffle, num_workers=num_workers)
        val = data.DataLoader(val, batch_size=2, shuffle = data_shuffle, num_workers=num_workers)
        return train, val
    elif purpose == 'Init':
        train = data.DataLoader(train, batch_size=2, shuffle = data_shuffle, num_workers=num_workers)
        return train