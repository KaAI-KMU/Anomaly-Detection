        ...
        for name in self.video_name:
            ...
            for start in range(seed, len(flow), int(self.args['segment_len']/2)):
                ...
                if end + self.args['pred_timesteps'] <= len(bbox) and end <= len(flow):
                    ...
                    input_flow = rec_plot(flow[start:end,:,:,:])
                    input_ego_motion = rec_plot(self.get_input(ego_motion, start, end))
                    
                    target_bbox = rec_plot(self.get_target(bbox, start, end))
                    target_flow = rec_plot(self.get_target(flow, start, end))
                    target_ego_motion = rec_plot(self.get_target(ego_motion, start, end))
