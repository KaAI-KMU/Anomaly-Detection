...
@click.argument('net_name', type=click.Choice(['mnist_LeNet', 'fmnist_LeNet', 'cifar10_LeNet', 'arrhythmia_mlp',
                                               ...
                                               'thyroid_mlp']))
@click.argument('xp_path', type=click.Path(exists=True), default = )
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model, eta,
    ...
    ae_dataset = load_dataset(dataset_name, data_path,)

    if n_known_outlier_classes > 1:
        logger.info('Known anomaly classes: %s' % (ae_dataset.known_outlier_classes,))
