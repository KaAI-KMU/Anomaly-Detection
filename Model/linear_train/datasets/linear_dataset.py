...
class Linear_Pretrain_DATASET(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_bbox = torch.flatten(torch.FloatTensor(input_bbox))
        input_flow = torch.flatten(torch.FloatTensor(input_flow))
        input_ego = torch.flatten(torch.FloatTensor(input_ego))

        return torch.cat((input_bbox, input_flow, input_ego)).to(device)
...
class Linear_SAD_DATASET(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.cat((input_bbox, input_flow, input_ego)).to(device), video_name, label, frame_id
