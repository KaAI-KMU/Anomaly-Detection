...
def init_weight(flow_model, bbox_model, ego_model, ego_model_ego_train, net_name, feature):
    ...
    # SAD 모델 생성
    SAD_other, SAD_ego = network_builder(net_name=net_name, feature = feature)
    ...
    SAD_other_c, SAD_ego_c = init_center_c(SAD_other, SAD_ego, net_name, feature)
...
def init_center_c(SAD_other, SAD_ego, net_name,feature, eps = 0.1):

    logger = logging.getLogger()
    logger.info(f'Start Initializing C point ::\t{net_name}')

    train_generator = dataset_loader(net_name, state = 'Init', purpose = 'Init')
    ...
    c_other = torch.zeros(feature, device=device)
