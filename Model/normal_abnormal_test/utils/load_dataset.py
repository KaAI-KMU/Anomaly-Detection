...
def dataset_loader(net_name, state = 'Pretrain', purpose = 'Train'):
    ...
    if net_name.split('_')[0] == 'Recurrence' and state == 'Pretrain':
        ...
        train = data.DataLoader(train, batch_size=data_batch, shuffle=True, num_workers=num_workers)
        val = data.DataLoader(val, batch_size=data_batch, shuffle=True, num_workers=num_workers)
    ...
    # Recurrence Model + SAD
        ...
        from datasets.Dataset_nor_abnor_recurrence import Recurrence_SAD_DATASET_NORMAL, Recurrence_SAD_DATASET_ABNORMAL
    ...
    # Recurrence Model + SAD + Ego Only
        ...
        from datasets.Dataset_nor_abnor_recurrence import Recurrence_SAD_EGO_DATASET_NORMAL, Recurrence_SAD_EGO_DATASET_ABNORMAL
    ...
    if purpose == 'Train':
        ...
        train_normal = data.DataLoader(train_normal, batch_size=data_batch//2, shuffle = data_shuffle, num_workers=num_workers)
        val_normal = data.DataLoader(val_normal, batch_size=data_batch//2, shuffle = data_shuffle, num_workers=num_workers)
        
        train_abnormal = data.DataLoader(train_abnormal, batch_size=data_batch//2, shuffle = data_shuffle, num_workers=num_workers)
        val_abnormal = data.DataLoader(val_abnormal, batch_size=data_batch//2, shuffle = data_shuffle, num_workers=num_workers)
