...
def main():
    ...
    res_addr = 'C:/Users/rosha/Desktop/Detection-of-Traffic-Anomaly-master/dataset/frames/0qfbmt4G8Rw_000435/'+images_path.split('/')[-3]
    #print('result address:', res_addr,tag)
    ...
    ego = np.zeros((len(path), 3))
    
    for i in range(1, len(path)):
        sv = np.sqrt(path[i,9]*path[i,9] + path[i,10]*path[i,10])
        yaw = np.arctan2(path[i,4], path[i,0])
        yaw22 = np.arctan2(-path[i,8], sv)
        #print(yaw)
        ego[i,0] = yaw
        ego[i,1] = path[i, 3]
        ego[i,2] = path[i, 11]
