...
def detect(save_img=False, line_thickness=1):
    source, weights, show_vid, save_txt, imgsz, trace, exp_name = opt.source, opt.yolo_weights, opt.show_vid, opt.save_txt, opt.img_size, opt.trace, opt.exp_name
    ...
    #exp_name=''  # save results to project/name
    ...
    source = os.path.join('D:/bdd100k/images/train/'+exp_name)
    source = str(source)
    
    # Directories
    save_dir = Path(increment_path(Path(opt.project) / exp_name, exist_ok=opt.exist_ok))  # increment run
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    txt_path = os.path.join('D:/bdd100k/bboxs/')
    txt_path = str(txt_path)
    
    temp_path = os.path.join(txt_path + exp_name +'.npy')
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + exp_name + '.npy', 'a') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_weights', nargs='+', type=str, default='weights/yolov7-tiny.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--exp_name', default='1f0af85d-386af660', help='save results to project/name')
