...
root = 'D:/Pickle_test_directory/'
base_save = 'D:/DoTA/result/'

video_names = [name for name in os.listdir(f'{root}dataset')]
...
def load_flo(video_name, frame_id, bbox):
    file_path = f'{root}dataset/{video_name}/{video_name}/{str(frame_id).zfill(6)}.flo'
...
for video_name in video_names:
    ...
    now_object = []
    dictionary = {}
    save_dir = f'{base_save}{video_name}/'

    if not os.path.isfile(f'{root}dataset/{video_name}/{video_name}.txt'):
        #logging that there isn.'t file using python logger
        logger.warning(f'Ego does not exit\t{root}dataset/{video_name}/{video_name}.txt')
    elif not os.path.isfile(f'{root}dataset/{video_name}/{video_name}.npy'):
        logger.warning(f'BBox does not exit\t{video_name}')
    else:
        if not os.path.isdir(save_dir):
            os.mkdir(save_dir)
        with open(f'{root}dataset/{video_name}/{video_name}.txt', 'r') as f:
            temp = f.read().strip()
        temp = temp.replace('\n',' ')
        ...
        bbox = np.loadtxt(f'{root}dataset/{video_name}/{video_name}.npy', delimiter=',')
